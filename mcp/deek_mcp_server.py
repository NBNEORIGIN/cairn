...
Specification: DEEK_MCP_SPEC.md
Start: python mcp/deek_mcp_server.py
...
DEEK_BASE_URL = (os.environ.get("DEEK_BASE_URL")
                 or os.environ.get("CAIRN_BASE_URL", "http://localhost:8765"))
API_KEY = (os.environ.get("DEEK_API_KEY")
           or os.environ.get("CAIRN_API_KEY")
           or os.environ.get("CLAW_API_KEY", "deek-dev-key-change-in-production"))
...
TOOLS = [
    types.Tool(
        ...
        description=(
            "Retrieve relevant code chunks from the Deek codebase index "
    ...
    types.Tool(
        ...
        description=(
            ...
            "Deek memory. Use to avoid repeating past mistakes, understand "
    ...
    types.Tool(
        name="update_memory",
        description=(
            "Write a memory entry to Deek after completing a task. Required "
            "after every development action that involved a decision, fix, or "
            "discovery. This is how Deek learns."
        ),
        inputSchema={
            "type": "object",
            "properties": {
    ...
    types.Tool(
        name="list_projects",
        description=(
            "List all projects currently loaded in Deek, with status. "
    ...
    types.Tool(
        ...
        description=(
            "Get the current health and status of Deek — API, file watcher, "
    ...
    try:
        async with httpx.AsyncClient(timeout=30.0, headers=HEADERS) as client:
            ...
            if name == "retrieve_codebase_context":
                ...
                r = await client.get(f"{DEEK_BASE_URL}/retrieve", params=params)
                ...
                r = await client.get(f"{DEEK_BASE_URL}/memory/retrieve", params=params)
                return [types.TextContent(type="text", text=r.text)]
                ...
                r = await client.post(f"{DEEK_BASE_URL}/costs/log", json=arguments)
                return [types.TextContent(type="text", text=r.text)]
        ...
        return [types.TextContent(
            type="text",
            text=json.dumps({
                "error": "Deek API is offline",
                "hint": "Start with: cd D:\\deek && .venv\\Scripts\\python -m uvicorn api.main:app --host 0.0.0.0 --port 8765",
            }),
        )]
