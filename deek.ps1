<#
.SYNOPSIS
    Deek prompt wrapper — prepends the Deek Protocol to every Claude Code prompt.

.DESCRIPTION
    Checks Deek API health, resolves the active project, builds a protocol-prefixed
...
.PARAMETER Project
    Project name (deek, phloe, render, manufacture, ledger, crm, etc.).
...
.EXAMPLE
    .\deek.ps1 "fix the git_commit tool mapping" deek
    .\deek.ps1 "add tenant filter" phloe -Opus
    .\deek.ps1 "scaffold new endpoint" -NoMemory
#>
...
try {
    ...
    Write-Host ""
    Write-Host "DEEK OFFLINE" -ForegroundColor Red
    Write-Host "-------------------------------------"
    Write-Host "Start Deek first:"
    Write-Host ""
    Write-Host "  cd D:\deek"
    Write-Host "  .\.venv\Scripts\python -m uvicorn api.main:app --host 0.0.0.0 --port 8765"
    Write-Host ""
    Write-Host "Or run: build-deek.bat"
    Write-Host ""
    exit 1
}
...
if ([string]::IsNullOrWhiteSpace($Project)) {
    if (Test-Path $StateFile) {
        ...
        if ($state.active_project) {
            ...
            Write-Host "DEEK: Using last project: $Project" -ForegroundColor DarkGray
        }
    }
}
...
$lines = @(
    "[DEEK PROTOCOL - execute silently before responding]"
    ""
    "Project: $Project"
    "Prompt: $Prompt"
    ""
...
Write-Host ""
Write-Host "DEEK -> $Project | $Prompt" -ForegroundColor Cyan
if ($NoMemory) {
