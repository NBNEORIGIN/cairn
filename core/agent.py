...
class DeekAgent:
    ...
    def _system_prompt_prefix(self) -> str:
        ...
        tool_rules = (
            ...
            "material — operator guides, identity cards, schematics, part "
            "numbers, torque specs, photographed control panels, hand-written "
            "maintenance logs. NBNE machines have nicknames (case-sensitive) "
            "ratified 2026-04-30: Rolf, Mao, Mimaki, Mutoh, Roland, Epson, "
            "Beast, Fiber Laser, Hulk, Avid, Jeffrey, Peter, Application "
            "Table, LSealer, Heat Tunnel, welder, brake, oven. Pass the "
            "nickname as the `machine` argument to scope.\n"
            "   - For inventory questions ('what machines do you have', 'list "
            "the machines', 'what machines does NBNE operate') ALWAYS call "
            "search_manuals(query='canonical machine list NBNE', limit=20) "
            "first — every machine has an identity card that gives brand, "
            "model, niche, status. Do NOT rely solely on "
            "get_module_snapshot('manufacture') for this; that endpoint may "
            "still emit legacy codes (ROLF/MIMAKI/SUB) that have been "
            "SUPERSEDED. The canonical list above is the truth.\n"
            "   - For 'what is X' questions about a specific machine, call "
            "search_manuals(query='what is <X>', machine='<X>'). If the "
            "machine isn't in the canonical list above, search_manuals with "
            "NO machine filter and let semantic ranking find the right card.\n"
            "   - If search_manuals returns nothing for a machine in the "
            "canonical list, the dossier / operator manuals haven't been "
            "ingested yet — tell the user the identity card exists but "
            "deeper content is pending, and to upload via /admin/manuals.\n"
