"""
Cairn federation + memory tools for the chat agent.
...
the chat agent direct access to:
  ...
  - Live module snapshots (chunk_type='module_snapshot'), refreshed every 15
    minutes by api/routes/cairn_federation.snapshot_poll_loop from each
    module's /api/cairn/snapshot endpoint.
  - Ingested email corpus (chunk_type='email'), written by
    scripts/process_cairn_inbox.py every 15 minutes via host cron.
...
Why these tools exist:
  ...
  Before today, the chat agent could only:
    ...
    (b) HTTP-fetch module context endpoints via web_fetch, which fails
        from inside the cairn-api container because localhost:<module_port>
...
def _get_conn():
    """Open a psycopg2 connection to the Cairn database.
            ...
            if not module:
                ...
                if not rows:
                    return (
                        ...
                        "cairn-api, or POST /api/cairn/poll-modules."
                    )
...
get_module_snapshot_tool = Tool(
    ...
    description=(
        ...
        "Cairn federation layer. Snapshots are refreshed automatically every "
