# Handover — deek_delegate Tool (brief from 2026-04-15)

**From:** closeout of Beacon Phase 1 session, 2026-04-15
**For:** fresh CC session picking up the `deek_delegate` brief
...
### 1. Deek MCP server is live and registered
- **File:** `D:\deek\mcp\deek_mcp_server.py` (336 lines, 7 tools today)
- **Registration:** `~/.claude/settings.json` points CC at it with `DEEK_API_KEY` env.
...
- **Implication for `deek_delegate`:** just add one more `types.Tool` entry + one
  more `elif name == "deek_delegate"` branch that POSTs to a new
...
- **File:** `D:\deek\core\models\openai_client.py` — full `OpenAIClient` with
...
- **File:** `D:\deek\core\models\router.py` (~320 lines)
- **Purpose:** routes between Ollama / DeepSeek / Claude / OpenRouter-fallback
  inside Deek's own agent loop using `TaskTier` + `ModelChoice` enums and
  hardware profile (`DEEK_HARDWARE_PROFILE`).
- **Implication:** do NOT extend this for `deek_delegate`. The existing
  router is about Deek's internal agent orchestration. `deek_delegate` is a
  new concern: external CC sessions delegating into Deek. Keep the new
...
- **Migration style:** Deek uses raw SQL migrations — check
  `D:\deek\core\memory\` and `D:\deek\data\` for patterns. SQLite file is
  per-project under `DEEK_DATA_DIR` (default `./data`).

---

## Blockers / open questions the fresh session must resolve before §0 step 2

### A. `OPENROUTER_API_KEY` is not set in `D:\deek\.env`
- `grep -i openrouter D:\deek\.env` returns nothing. The `.env.example` also
  has no OR line.
- The 5 consumer files I listed above all `os.getenv('OPENROUTER_API_KEY', '')`
  and handle the empty case — so Deek today effectively has no live
  OpenRouter path, just stubs.
- Toby's brief says "the api key is in the deek / cairn .env files" but it
...
### D. Deek MCP server does NOT appear to be live in CC sessions today
- `~/.claude/settings.json` registers it, but the CC session I ran Beacon
  ...
  the Deek FastAPI directly instead.
- Unclear whether the Python MCP server failed to start, or CC's tool-search
  ...
  session start).
- **The fresh session will not be able to `call_tool` `deek_delegate` at
  all unless the MCP server is actually connecting to CC.** Diagnose this
  before §1 acceptance testing. Start by running:
  ```
  D:\deek\.venv\Scripts\python.exe D:\deek\mcp\deek_mcp_server.py
  ```
...
### E. The `cairn` project isn't registered inside Deek itself
- `D:\deek\projects\` has `deek`, `phloe`, `render`, `beacon`, etc. — but no
  `cairn`. The `deek` project IS Deek's own self-reference.
- The brief's memory write-backs use `project="cairn"`. Either:
  - Use `project="deek"` (existing convention, `deek` IS Deek), OR
  - Register a new `cairn` project first
- **Recommend:** use `project="deek"`. The memory is already semantic-
  searchable across all projects; splitting cairn-vs-deek creates two
...
2. Read `D:\deek\mcp\deek_mcp_server.py` fully (it's the template for where
   the new tool plugs in).
3. Read `D:\deek\api\main.py` lines 1–120 (FastAPI app setup, route wiring
...
5. Ask Toby whether `OPENROUTER_API_KEY` is set somewhere non-obvious, or
   whether he'll add it to `D:\deek\.env` before §0 step 2 (blocker A
   above).
...
- Did not commit or push anything in `D:\deek` for this brief.
...
None. `D:\deek\git status` unchanged except for this new doc file, which
...
Key is live in `D:\deek\.env` as `OPENROUTER_API_KEY=...` (single word —
...
## E — Project name for memory write-backs: use `project="deek"`

Confirmed with Toby. Brief's `project="cairn"` was a mistake — `deek`
is the codebase, Deek is the ecosystem brand; memory write-backs key
off codebase. `/projects` endpoint on Deek API confirms:
- `deek` — `has_config: true, has_core_md: true, ready: true`
...
`project="cairn"` with `project="deek"`. All D-log entries, retrieval
calls, and write-backs use `"deek"`. Do NOT register a `cairn` project.

## D — Deek MCP server diagnosis: Outcome #1 (restart required)
...
1. `~/.claude/settings.json` registers the server correctly under
   `mcpServers.cairn` with command `D:\deek\.venv\Scripts\python.exe`,
   args `["D:\deek\mcp\deek_mcp_server.py"]`, env
   `DEEK_API_KEY=deek-dev-key-change-in-production`. No config change
   needed.
2. Deek FastAPI backend is live: `GET http://localhost:8765/health`
   returned 200.
...
Deek API is running (it is — leave it alone). On session start, verify
...
Interim fallback while MCP unavailable: call the Deek FastAPI
endpoints directly via `curl` with `X-API-Key:
deek-dev-key-change-in-production` header. Same semantics, just no MCP
schema enforcement.
...
| E       | Confirmed. Use `project="deek"` throughout.                    |
...
Deek's existing layout. Both surfaced before any cleanup or build work.

## F — D-101 (where the new code lives in deek): prescribe, don't guess

The brief says §0 step 1 is "find the right place in deek for the new
code … likely `deek/delegation/` as a new package, but check existing
layout conventions."

Deek's existing layout is already clear:
- **MCP server** (`D:\deek\mcp\deek_mcp_server.py`) — thin JSON-RPC
  wrapper. No business logic. Forwards to FastAPI via `X-API-Key`.
- **FastAPI backend** (`D:\deek\api\main.py`) — route handlers that call
  into business-logic modules under `D:\deek\core\`.
- **Business logic** lives under `D:\deek\core\<domain>\` (e.g.
  `core/models/`, `core/memory/`, `core/social/`, `core/wiki/`).

So the new code lands:
- `D:\deek\core\delegation\openrouter_client.py` — thin httpx wrapper
  for the OpenRouter chat-completions endpoint. One `call()` function.
- `D:\deek\core\delegation\router.py` — NEW file, the task_type →
  model routing rule per D-C. NOT to be confused with
  `core/models/router.py` (internal Deek agent orchestration —
  different concern, see §3 above). Add a one-line comment at the top
  noting the distinction.
- `D:\deek\core\delegation\cost.py` — pricing constants, USD→GBP
...
- **MCP tool declaration** goes in the existing
  `D:\deek\mcp\deek_mcp_server.py` as one new `types.Tool` entry +
...
brief's "deek/delegation/" (top-level package) would break Deek's
...
to reference `deek_delegate` as the enforcement mechanism. One-line
guide on when to use it."

`CLAUDE.md` Rule 1 already exists. Current text (see `D:\deek\CLAUDE.md`
STEP 2b):
...
This Rule 1 is about **in-house delegation** — Claude-in-Deek
delegating to Qwen/DeepSeek inside Deek's own agent loop. It is NOT
about **cross-module delegation** — CC sessions in a module (Beacon,
Phloe, Render, CRM) calling `deek_delegate` to push work to
...
"Rule 1b — Cross-module delegation via `deek_delegate`" under
