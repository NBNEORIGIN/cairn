...
Read `DEEK_PROTOCOL.md`, `CLAUDE.md`, `DEEK_MODULES.md`, and `docs/deek/DEEK_AMAZON_LISTINGS_CC_PROMPT.md` before starting. This session covers Phases 0–2 of that brief, modified by Toby's feedback below.
...
| Deek context endpoint | `/ami/cairn/context` | Working — feeds chat agent |
...
All three regions authenticate. Tokens are in `/opt/nbne/deek/deploy/.env` on Hetzner. Seller IDs have hardcoded fallbacks in `client.py`.

## AWS status

- **Account:** NBNE (9150-7785-2106), us-east-1
- **AWS Console access:** Toby has it (IAM, S3, IoT Core recently visited)
- **Current AWS usage in Deek:** Zero. No boto3, no SQS, no IAM credentials in the codebase.
- **What Session 1 needs to set up:** IAM user with SQS permissions, three SQS queues (eu-west-2, us-east-1, ap-southeast-2), IAM role for Amazon's notification service. Document every step in `docs/deek/amazon_notifications_setup.md`.
...
Before Phase 5 (nuke) can run in Session 2, produce a `docs/deek/amazon_consumers.md` that maps every reference to the old module:
...
- **Deek API:** `deploy-deek-api-1` at `178.104.1.152`, port 8765
- **Deek DB:** `deploy-deek-db-1`, PostgreSQL 16 with pgvector
- **Deploy env:** `/opt/nbne/deek/deploy/.env`
- **Hot-patch pattern:** `docker cp` file into container + `docker compose restart deek-api`
- **Full rebuild:** `cd /opt/nbne/deek/deploy && bash build-deek-api.sh full`
...
8. `docs/deek/amazon_notifications_setup.md` documenting the AWS setup
9. `docs/deek/amazon_consumers.md` with the consumer inventory (prep for Session 2)
