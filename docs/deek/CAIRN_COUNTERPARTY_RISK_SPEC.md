# DEEK_COUNTERPARTY_RISK_SPEC.md
**Deek Service Specification: Counterparty Risk & Terms Engine**
Version: 0.2
Date: 14 April 2026
Owner: Toby Fletcher
...
This is the specification for a Deek service that maintains
...
The service is consumed by the PM sub-agent (DEEK_PM_AGENT_SPEC.md)
...
| Evasive on direct factual questions | Direct factual question (price, date, scope, documentation) met with deflection, generalisation, or topic change | 3 | Annotated by Deek LLM with evidence quote |
| Generalised complaints without specifics | Complaint registered (cost, quality, expectations) without specific itemisation when asked | 2 | Annotated by Deek LLM |
...
| Signal | Definition | Weight | Detection |
|---|---|---|---|
| Grandiose / exceptional claims | Unverifiable claims of unique status, market firsts, exclusive access, special licences | 2 | Annotated by Deek LLM |
...
fatigue, partial commercial entanglement) and where Deek earns its
keep.
...
- Stored outside Deek (not in any database Deek has access to)
...
The mapping key is never accessed by Deek. Sub-agents (PM,
...
This data is the most sensitive in Deek. Access controls are
...
Per DEEK_PM_AGENT_SPEC.md, the PM agent now consumes this service
on three occasions:
...
2. Deek observer scans for new signal events; if any are detected
...
This prompt is used by the Deek observer (the LLM-backed component
...
The observer prompt must be compiled at runtime by the Deek API,
...
stored in Deek's prompt registry alongside the PM agent prompt and
other sub-agent prompts, following the same compilation pattern used
by `D:\deek\api\routes\deek_federation.py` for module snapshots.

```
You are the Counterparty Observer for Deek, the AI substrate serving
...
- [ ] Deek API endpoint: `GET /api/counterparty-risk/{id}` returning
...
### Phase 2 — Deek observer + automatic signal detection (3 weeks)
