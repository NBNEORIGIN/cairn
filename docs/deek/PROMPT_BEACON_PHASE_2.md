...
Standard Deek protocol applies. Memory write-back with non-empty
`rejected`, `project="deek"`.
...
`Bearer <DEEK_API_KEY>` header (see D-008). Fix by adding
...
Verify with:
```
curl -s -H "Authorization: Bearer <DEEK_API_KEY>" http://localhost:8017/api/cairn/context
```
