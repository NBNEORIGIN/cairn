...
```bash
curl -s -H "Authorization: Bearer $DEEK_API_KEY" http://localhost:8017/api/cairn/context | python -m json.tool
```
...
and that `DEEK_API_KEY` in the environment matches what you're sending.
