"""
Deek module federation routes — mounted at /api/cairn/*.

Implements the snapshot-pattern federation described in DEEK_MODULES.md:
  ...
  - Deek polls each registered module on a fixed interval, pulls the
...
Background task:
  ...
  DEEK_SNAPSHOT_INTERVAL_MINUTES (default 15). Registry file is
...
router = APIRouter(tags=["deek-federation"])

_DEEK_ROOT = Path(__file__).resolve().parents[2]
_REGISTRY_PATH = _DEEK_ROOT / "deploy" / "modules.json"
...
async def ingest_health(_: bool = Depends(verify_api_key)) -> dict[str, Any]:
    """
    Freshness diagnostic for Deek's indexed content.

    Returns the max indexed_at for each chunk_type on the 'deek' project,
...
@router.get("/context")
async def deek_context(_: bool = Depends(verify_api_key)) -> dict[str, Any]:
    """Deek's own context snapshot for cross-module federation.
...
async def snapshot_poll_loop() -> None:
    ...
    Polls every DEEK_SNAPSHOT_INTERVAL_MINUTES (default 15). Sleeps first
    so Deek startup isn't blocked on module availability.
    """
    interval_min = int(os.getenv("DEEK_SNAPSHOT_INTERVAL_MINUTES")
                       or os.getenv("CAIRN_SNAPSHOT_INTERVAL_MINUTES", "15"))
    if interval_min <= 0:
