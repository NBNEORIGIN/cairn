# DEEK WIKI LAYER — Implementation Prompt
...
Pull memory for project "deek" before starting.
Read the original spec: D:\deek\projects\deek\DEEK_WIKI_CC_PROMPT_ORIGINAL.md
This prompt supersedes it with corrections.

This brief modifies Deek's core retrieval architecture — understand the
...
1. **Etsy Intelligence is missing from the graph.** Add it as a `sell`
   category node with edges to Deek and Manufacture. 8 nodes, not 7.
...
3. **No local Ollama on Hetzner.** The Deek API on Hetzner runs
   `DEEK_FORCE_API=true` — no Ollama. Wiki compilation must use:
   - **DeepSeek** via existing API key (cheapest, ~£0.20/1M input)
   - **OpenRouter** as fallback (access to Llama 70B, Mixtral, etc.)
   OpenRouter API key: pull from Deek memory (reference_openrouter_api.md)
...
4. **Phloe article content is wrong.** Do NOT hardcode the article from
   the spec. Instead:
   - Pull current tenant data from Deek memory and Phloe context endpoint
...
5. **`manage.py` is Django — Deek is FastAPI.** The compilation schedule
   ...
   - Or use Deek's existing scheduling infrastructure
...
The wiki's module articles also power a visual map on deek.nbnesigns.co.uk
...
### Database
PostgreSQL on nbne1 (192.168.1.228), database: deek
...
### Hetzner deployment
Deek API: 178.104.1.152, port 8765 (Docker container deploy-deek-api-1)
After changes: `ssh root@178.104.1.152 "cd /opt/nbne/deek && git pull && cd deploy && docker compose --env-file .env up -d --build deek-api"`

### Web interface
deek.nbnesigns.co.uk, web-business container
After changes: rebuild and restart deek-web
...
OPENROUTER_API_KEY=<from Deek memory>
```
...
Before each section:
  retrieve_codebase_context(query=<section>, project="deek", limit=10)
  retrieve_chat_history(query=<section>, project="deek", limit=10)

After each section:
  update_memory(project="deek", query=..., decision=..., rejected=...,
                outcome="committed", model=..., files_changed=[...])

## Cost Logging

After every section:
  log_cost(session_id=<session>, prompt_summary=<one line>, project="deek",
...
and Deek memory — do NOT copy the Phloe article from the spec verbatim
...
Add /map route to deek.nbnesigns.co.uk. Add to sidebar navigation.
...
Add /map to sidebar navigation on deek.nbnesigns.co.uk.
...
Rebuild both deek-api and deek-web. Test:
...
- Write back to Deek memory after each section
