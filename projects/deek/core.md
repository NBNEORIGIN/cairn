# Deek — Self-Hosted Sovereign AI Agent
# Version: 1.1
# This is DEEK's context for maintaining itself.

## What this is
DEEK (Coding and Language Agent Workbench) is a self-hosted AI coding agent
built at D:\deek. It replaces Cursor/Windsurf as the primary AI coding tool
...
├── core/
│   ├── agent.py      Main orchestrator — DeekAgent.process()
...
│   ├── deek/         This project (self-referential)
│   ├── phloe/        Phloe WaaS platform
│   └── manufacturing/ Origin Designed manufacturing app
├── core/wiggum.py    WiggumOrchestrator — outer loop driving DEEK toward a goal
├── tests/            test_deek.py — 36 pytest tests (run: .venv\Scripts\python -m pytest tests/)
├── web/              Next.js chat UI — runs on port 3000 (next dev)
├── tray/             deek_tray.py — system tray process manager (replaces NSSM)
├── vscode-extension/ VS Code panel — Ctrl+Shift+A
└── scripts/          test_agent.py, index_project.py, new_project.py
```

## Running services
- DEEK API: managed by tray app — http://localhost:8765
...
## WIGGUM loop
- POST /wiggum  {"goal": "...", "success_criteria": [...], "project": "deek"}
...
## Indexing status
- DEEK self-index: ✅ BUILT — 69 files, 0 errors (pgvector, nomic-embed-text)
- Phloe index: ✅ BUILT — 1,902 chunks across 260 files, 0 errors
- Tier 2 semantic search is ACTIVE for both projects
- Re-index after major changes: python scripts/index_project.py --project deek
...
## Key files to know
- core/agent.py — DeekAgent, tool dispatch, model routing, _get_api_client()
...
- .env — API keys, model config, API_PROVIDER, DEEK_FORCE_API
- tests/test_deek.py — 27 tests covering endpoints, tools, models, indexer
...
4. The venv is at D:\deek\.venv — always use .\.venv\Scripts\activate on Windows
5. Ports: API=8765, Web UI=3000

## Session Log

### Session 2026-03-24 — deek
...
- pgvector index built and active for both deek and phloe projects
...
## Session 2026-03-28 — deek (bc3db364)
...
## Session 2026-03-28 — deek (1973b4d9)
...
## Session 2026-03-28 — deek (06985e28)
...
## Session 2026-03-28 — deek (4aaf39ef)
...
## Session 2026-03-29 — deek (9adf340a)
...
## Session 2026-04-15 — deek (deek_delegate Commit 6)
- D-102: Exposed `/api/delegation/` on deek.nbnesigns.co.uk via nginx location block on Hetzner (178.104.1.152). Minimum-surface approach (Option 1).
...
- Verified publicly: `POST https://deek.nbnesigns.co.uk/api/delegation/call` returns 401 without `X-API-Key`, 422 with valid key + empty body. Auth uses `DEEK_API_KEY` (middleware env var) not `DEEK_API_KEY`.

## D-103 — deek_delegate dogfooding observation (2026-04-15)
...
**Cost:**
- deek_delegate calls used: 1 (plus one £0.00 ping for connectivity; no retries).
...
Grok Fast via `deek_delegate` produced acceptable production code on the first attempt for a well-specified SQLite aggregation helper. Two small bugs (one contract violation, one type drift) were caught by Sonnet review and fixed in under three minutes. Total delegation cost was £0.0022 against a rough self-execution cost of £0.05. The tool works for the workflow it was designed for: cheap tier writes a narrow-scope function from a detailed spec, Sonnet reviews, Sonnet either accepts, tweaks, or rewrites. The recommendation for future module sessions: use `deek_delegate` for discrete helper functions, SQL query builders, and schema-stable extraction work where the spec can be written out in <500 words. Do not use it for multi-file refactors, cross-module design decisions, or anything requiring holding invariants across the codebase.
...
- `deek_delegate` MCP tool wrapper in `mcp/deek_mcp_server.py` is separately needed — decision to remain HTTPS REST-only is closed per handover D-B/D-D. If that decision is revisited, the wrapper is a one-tool-entry change.
