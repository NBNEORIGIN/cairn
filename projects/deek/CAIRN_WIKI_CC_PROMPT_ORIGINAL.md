# DEEK WIKI LAYER — Claude Code Implementation Brief
...
Deek currently retrieves raw chunks from pgvector. Chunks are paragraphs
...
Additionally, the wiki's module articles power a visual map on the Deek
...
Create the wiki directory within the Deek project:
...
Use the information in `projects/*/core.md` files and DEEK_PROTOCOL.md
...
The categories follow the business value chain from DEEK_MODULES.md:
**Make → Measure → Sell**, with Deek as the brain that connects them.
...
Build a React component for the Deek web interface that renders the
...
- Accessible at `/map` on deek.nbnesigns.co.uk
...
```markdown
## Wiki Layer
Deek maintains a compiled wiki at `wiki/`. When retrieving context,
...
const wikiResults = await fetch(
  `${DEEK_API}/api/wiki/search?q=${encodeURIComponent(userMessage)}&top_k=3`
);
...
Write to Deek memory after each compilation run.
...
After Section 2 (module articles): write to `deek/core.md`
After Section 4 (visual map): write to `deek/core.md`
After Section 6 (retrieval boost): write to `deek/core.md` at Opus level
