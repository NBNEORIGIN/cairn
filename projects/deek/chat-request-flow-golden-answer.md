# DEEK Chat Request Flow — Golden Answer

Use this as the reference answer for the prompt:

`Read the DEEK codebase and explain how chat requests flow from the web UI to the model response. Name the main files involved and point out where model routing, context retrieval, validation, and streaming happen.`

## End-to-End Flow

### 1. Web UI entry points

The browser-side chat experience lives in [web/src/components/ChatWindow.tsx](D:/deek/web/src/components/ChatWindow.tsx).
...
- [web/src/app/api/chat/route.ts](D:/deek/web/src/app/api/chat/route.ts)
  This forwards POST requests to FastAPI `/chat` and adds the API key header.
- [web/src/app/api/chat/stream/route.ts](D:/deek/web/src/app/api/chat/stream/route.ts)
  This forwards SSE requests to FastAPI `/chat/stream` and streams the body back to the browser.

### 2. FastAPI request handling

The main backend entry points are in [api/main.py](D:/deek/api/main.py).
...
The envelope shape is defined in [core/channels/envelope.py](D:/deek/core/channels/envelope.py).

### 3. Agent orchestration

The main orchestration happens in [core/agent.py](D:/deek/core/agent.py).

`DeekAgent.process()` does the non-streaming flow:
...
`DeekAgent.process_streaming()` follows the same logic but yields SSE events like:
...
Context retrieval is implemented in [core/context/engine.py](D:/deek/core/context/engine.py).
...
Routing is implemented in [core/models/router.py](D:/deek/core/models/router.py).

The current entry point is `route_decision(...)`, not just `route()`.

It uses:

- the task classifier from [core/models/task_classifier.py](D:/deek/core/models/task_classifier.py)
- project config overrides like `force_model`
- per-message `force_tier` derived from `model_override`
- project rules such as `DEEK_TIER4_PROJECTS`
...
- [core/models/claude_client.py](D:/deek/core/models/claude_client.py)
- [core/models/deepseek_client.py](D:/deek/core/models/deepseek_client.py)
- [core/models/openai_client.py](D:/deek/core/models/openai_client.py)
- [core/models/ollama_client.py](D:/deek/core/models/ollama_client.py)

`_chat_with_fallback(...)` in [core/agent.py](D:/deek/core/agent.py) wraps provider calls, applies a hard timeout, and falls back on retryable failures or timeouts.

### 7. Validation

Post-response validation lives in [core/models/output_validator.py](D:/deek/core/models/output_validator.py).
...
`_validate_final_response(...)` in [core/agent.py](D:/deek/core/agent.py) can:
...
- frontend SSE consumer:
  [web/src/components/ChatWindow.tsx](D:/deek/web/src/components/ChatWindow.tsx)
- Next.js SSE proxy:
  [web/src/app/api/chat/stream/route.ts](D:/deek/web/src/app/api/chat/stream/route.ts)
- FastAPI stream endpoint:
  [api/main.py](D:/deek/api/main.py)
- streaming generator:
  [core/agent.py](D:/deek/core/agent.py)
...
→ `api/main.py`
→ `MessageEnvelope`
→ `DeekAgent.process()` / `process_streaming()`
...
A good DEEK answer to the prompt should:
