# CLAW — WIGGUM Micro Loop Prompt: Eval, Streaming, Memory, Skills

Use this prompt for the next CLAW hardening milestone.

This run must implement the following five items in a way that fits the
current CLAW architecture. Do not rewrite the request path around a new
provider abstraction. Work with the existing `ClawAgent`, `ContextEngine`,
and web SSE flow.

## Goal

Make CLAW measurably better at:
...
- Add a disk-backed prompt suite for CLAW under `projects/claw/`.
...
- `projects/claw/chat-request-flow-golden-answer.md` remains a valid reference.
