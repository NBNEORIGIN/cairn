...
Run `build-deek.bat` from D:\deek. This starts:
- The Deek API on port 8765
- The MCP server (if built)

Wait for the terminal windows to show they are running before sending prompts.

If Deek is already running, you do not need to start it again.
...
cairn "list all projects and their status" deek
```
...
| Name | What it is |
|---|---|
| deek | Deek itself (the AI development system) |
...
```
cairn "scaffold a brand new module" deek -NoMemory
```
...
type `/init` to re-anchor the Deek protocol. This re-reads all instructions,
...
Deek logs the cost of every prompt.

**Quick check**: Open `D:\deek\data\cost_log.csv` in Excel or any text editor.
...
If you get a "DEEK OFFLINE" message:

1. Open a terminal
2. Navigate to D:\deek
3. Run: `build-deek.bat`
...
- Is PostgreSQL running? (Deek needs it for memory and embeddings)
- Is the .env file present at D:\deek\.env?
...
Deek gets smarter every time it is used. Every prompt that writes back to
