...
| `/opt/nbne/deek/deploy/` | Deek (FastAPI + Next.js) |
...
| deek.nbnesigns.co.uk | `deploy` | 8765/3000 | `/opt/nbne/deek/deploy/` |
...
Deek runs on `deploy_default` Docker network. Manufacture backend joins `cairn_net` (external alias for `deploy_default`) so they can talk container-to-container. However, Deek is started with `--host 0.0.0.0` but cross-network HTTP may timeout due to iptables — use direct SP-API calls from Manufacture rather than relying on Deek HTTP.
