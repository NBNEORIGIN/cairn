...
Deek exposes its memory and retrieval capabilities as an MCP (Model Context Protocol)
server. This allows Claude Code, Codex, and any other MCP-compatible head model to
treat Deek's memory as native tools — indistinguishable from file reads or git
operations.

The MCP server wraps Deek's existing FastAPI endpoints at http://localhost:8765.
...
```
D:\deek\mcp\deek_mcp_server.py
```

Run alongside the Deek API:
```powershell
cd D:\deek
python mcp\deek_mcp_server.py
```

Or add to `build-deek.bat` so it starts automatically with the API.
...
```json
{
  ...
  "description": "Retrieve relevant code chunks from the Deek codebase index using hybrid BM25 + semantic search. Use before starting any development task to surface prior decisions, related code, and architectural context.",
  "input_schema": {
    "type": "object",
    "properties": {
      "query": {
      ...
      "project": {
        "type": "string",
        "description": "Project name to scope the search. One of: deek, phloe, render, crm, bookkeeping, studio, houseofhair, clayport, a1g"
      },
      "limit": {
        "type": "integer",
...
```json
{
  ...
  "description": "Retrieve relevant development decisions and chat history from Deek memory. Use to avoid repeating past mistakes, understand prior approaches, and maintain continuity across sessions.",
  "input_schema": {
    "type": "object",
    "properties": {
      "query": {
...
Writes a memory entry to Deek after completing a task. This is the write-back
tool. Call this after every non-trivial action.

```json
{
  "name": "update_memory",
  "description": "Write a memory entry to Deek after completing a task. Required after every development action that involved a decision, fix, or discovery. This is how Deek learns.",
  "input_schema": {
    "type": "object",
    "properties": {
      "project": {
...
Returns all projects currently loaded in Deek with their indexing status.

```json
{
  "name": "list_projects",
  "description": "List all projects currently loaded in Deek, with file counts, chunk counts, and index status. Use at session start to confirm which projects are available.",
  "input_schema": {
    "type": "object",
    "properties": {},
    "required": []
...
Returns the health and status of Deek itself — API, file watcher, model
availability, and memory entry counts.

```json
{
  "name": "get_project_status",
  "description": "Get the current health and status of Deek — API, file watcher, model tiers, and memory. Use at session start to confirm the system is ready before beginning work.",
  "input_schema": {
    "type": "object",
    "properties": {
      "project": {
...
2026-03-30T08:00:00Z,sess_001,deek,fix git_commit tool,qwen2.5-coder:32b,0,0,0.000000,0.003240
2026-03-30T08:00:00Z,sess_001,deek,fix git_commit tool,deepseek-chat,1200,480,0.003240,0.003240
```
...
Build `D:\deek\mcp\deek_mcp_server.py` as a thin wrapper:

```python
"""
Deek MCP Server
Wraps Deek FastAPI endpoints as MCP tools for Claude Code and compatible clients.
"""
...
DEEK_BASE_URL = "http://localhost:8765"
server = Server("cairn")

@server.list_tools()
async def list_tools():
    # Return all 5 tool definitions as defined in DEEK_MCP_SPEC.md
    ...
...
async def call_tool(name: str, arguments: dict):
    async with httpx.AsyncClient(timeout=30.0) as client:
        if name == "retrieve_codebase_context":
            r = await client.get(f"{DEEK_BASE_URL}/retrieve", params=arguments)
            return [types.TextContent(type="text", text=r.text)]

        elif name == "retrieve_chat_history":
            r = await client.get(f"{DEEK_BASE_URL}/memory/retrieve", params=arguments)
            return [types.TextContent(type="text", text=r.text)]

        elif name == "update_memory":
            r = await client.post(f"{DEEK_BASE_URL}/memory/write", json=arguments)
            return [types.TextContent(type="text", text=r.text)]

        elif name == "list_projects":
            r = await client.get(f"{DEEK_BASE_URL}/projects")
            return [types.TextContent(type="text", text=r.text)]

        elif name == "get_project_status":
            params = {"project": arguments["project"]} if "project" in arguments else {}
            r = await client.get(f"{DEEK_BASE_URL}/health", params=params)
            return [types.TextContent(type="text", text=r.text)]
...
```json
{
  "mcpServers": {
    "cairn": {
      "command": "python",
      "args": ["D:\\deek\\mcp\\deek_mcp_server.py"],
      "env": {}
    }
  }
}
...
This MCP server should be built by CC in the next Deek development session,
after the git_commit tool fix. It is the most important infrastructure improvement
available — it makes Deek's memory native to any MCP-compatible head model rather
