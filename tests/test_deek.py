"""
DEEK test suite — ~20 tests covering:
...
def client(auth_headers):
    ...
    with patch("core.models.claude_client.ClaudeClient.chat", new_callable=AsyncMock) as mock_chat, \
        ...
        for project_dir in sorted(projects_root.iterdir()):
            ...
            agent = DeekAgent(project_dir.name, config)
...
class TestProjects:
    ...
    def test_projects_includes_claw(self, client):
        ...
        assert "deek" in ids, f"Expected 'deek' in {ids}"

    def test_projects_claw_is_ready(self, client):
        tc, headers = client
        projects = tc.get("/projects", headers=headers).json()["projects"]
        deek = next(p for p in projects if p["id"] == "deek")
        assert deek["ready"] is True
        assert deek["has_config"] is True
        assert deek["has_core_md"] is True
...
class TestChatValidation:
    def test_chat_requires_auth(self, client):
        tc, _ = client
        r = tc.post("/chat", json={"content": "hi", "project_id": "deek",
                                   "session_id": "s1"})
        assert r.status_code == 401
    ...
    def test_chat_missing_content_returns_422(self, client):
        ...
        r = tc.post("/chat", headers=headers,
                    json={"project_id": "deek", "session_id": "s1"})
    ...
    def test_chat_missing_session_id_returns_422(self, client):
        tc, headers = client
        r = tc.post("/chat", headers=headers,
                    json={"content": "hi", "project_id": "deek"})
        assert r.status_code == 422
    ...
    def test_chat_returns_expected_keys(self, client):
        tc, headers = client
        r = tc.post("/chat", headers=headers,
                    json={"content": "What is DEEK?",
                          "project_id": "deek", "session_id": "test-session-1"})
        assert r.status_code == 200
        data = r.json()
...
class TestDebugTools:
    def test_debug_tools_returns_list(self, client):
        tc, _ = client
        data = tc.get("/debug/tools/deek").json()
...
def populated_registry():
    """Return a ToolRegistry with all tools registered via a DeekAgent."""
    import json
    from pathlib import Path
    from core.agent import DeekAgent
    config = json.loads((Path("projects/deek/config.json")).read_text())
    agent = DeekAgent("deek", config)
    return agent.tools
...
class TestModelRouter:
    def test_force_api_true_never_returns_local(self):
        """DEEK_FORCE_API=true bypasses Ollama — result is DeepSeek or Claude, not LOCAL."""
        os.environ["DEEK_FORCE_API"] = "true"
        import importlib
    ...
    def test_route_decision_marks_tier4_project_as_opus(self):
        ...
        try:
            ...
            del os.environ["DEEK_TIER4_PROJECTS"]
...
class TestOpenAIClient:
    ...
    def test_build_messages_includes_system(self):
        ...
        client = self._make_client()
        msgs = client._build_messages("You are DEEK.", [], "Hello")
        assert msgs[0]["role"] == "system"
        assert msgs[0]["content"] == "You are DEEK."
...
class TestWiggum:
    ...
    def test_wiggum_evaluate_success_all_pass(self):
        ...
        w = WiggumOrchestrator("goal", ["a", "b"], "deek", "http://x", "key")
    ...
    def test_read_only_limits_tools(self, populated_registry):
        ...
        from core.agent import DeekAgent
        config = _json.loads(
            ((__import__('pathlib').Path('projects/deek/config.json')).read_text())
        )
        agent = DeekAgent('deek', config)
    ...
    def test_explanation_prompt_limits_tools_to_safe(self):
        ...
        config = _json.loads(Path('projects/deek/config.json').read_text())
        agent = DeekAgent('deek', config)
        tools = agent._get_tools_for_task(
            'Read the DEEK codebase and explain how chat requests flow from the web UI to the model response.',
            read_only=False,
        )
    ...
    def test_queued_tool_fallback_response_is_not_blank(self):
        ...
        assert 'tell me about deek' in text
...
class TestModelRouterDeepSeek:
    ...
    def test_tier_priority_local_beats_deepseek(self):
        """Ollama (Tier 1) takes priority over DeepSeek when DEEK_FORCE_API=false."""
        self._set_env(
            DEEPSEEK_API_KEY="sk-ds-test",
            DEEPSEEK_ENABLED="true",
            DEEK_FORCE_API="false",
        )
        import importlib
        from core.models import router
        importlib.reload(router)
        ...
        assert result == router.ModelChoice.LOCAL
        self._set_env(DEEK_FORCE_API="true")  # restore
...
class TestModelRouterTierPromotion:
    ...
    def test_tier4_projects_forces_api(self):
        """DEEK_TIER4_PROJECTS routes project to API regardless of task."""
        self._set_env(DEEK_TIER4_PROJECTS='phloe,manufacturing', DEEK_FORCE_API='true',
        ...
        assert result == router.ModelChoice.API
        self._set_env(DEEK_TIER4_PROJECTS=None)
    ...
    def test_skips_unavailable_deepseek_promotes_to_api(self):
        ...
        self._set_env(DEEPSEEK_API_KEY=None, DEEK_FORCE_API='true')
        import importlib
        from core.models import router
        importlib.reload(router)
...
class TestOutputValidator:
    ...
    def test_check3_hallucinated_file_fails(self):
        ...
        r = validate("See core/nonexistent_module.py for details.",
                     files_in_context=[], project_root='D:/deek')
        assert any('CHECK 3' in f for f in r.failures)

    def test_check3_ignores_framework_names(self):
        from core.models.output_validator import validate
        r = validate("DEEK uses Next.js for the web UI and FastAPI for the API.",
                     files_in_context=[], project_root='D:/deek')
...
class TestAgentHardening:
    ...
    def test_process_includes_memory_metadata(self):
        ...
        agent.claude.chat = AsyncMock(return_value=(
            "DEEK is a sovereign coding agent.",
            None,
    ...
    def test_explanation_prompts_use_lower_tool_round_cap(self):
        ...
        envelope = MessageEnvelope(
            content="Read the DEEK codebase and explain how chat requests flow from the web UI to the model response.",
            channel=Channel.WEB,
            project_id="deek",
    ...
    def test_validate_final_response_uses_tool_summary_fallback_after_empty_failures(self):
        ...
        agent = DeekAgent("deek", config)
        envelope = MessageEnvelope(
            content="Read the DEEK codebase and explain the chat flow.",
            channel=Channel.WEB,
            project_id="deek",
        ...
        assert "Here are the main sources DEEK inspected" in text
...
class TestWiggumBatchMode:
    ...
    def test_wiggum_orchestrator_batch_mode_default_false(self):
        ...
        from core.wiggum import WiggumOrchestrator
        orc = WiggumOrchestrator(goal='test', success_criteria=['x'], project_id='deek')
    ...
    def test_wiggum_orchestrator_batch_mode_true(self):
        ...
        orc = WiggumOrchestrator(goal='test', success_criteria=['x'],
                                  project_id='deek', batch_mode=True)
...
class TestSubprojectEndpoints:
    def test_get_subprojects_endpoint(self, client):
        tc, headers = client
        r = tc.get('/projects/deek/subprojects', headers=headers)
        assert r.status_code == 200
        data = r.json()
    ...
    def test_create_subproject_endpoint(self, client):
        tc, headers = client
        r = tc.post('/projects/deek/subprojects', headers=headers, json={
    ...
    def test_create_subproject_endpoint_idempotent(self, client):
        ...
        r1 = tc.post('/projects/deek/subprojects', headers=headers, json=body)
        r2 = tc.post('/projects/deek/subprojects', headers=headers, json=body)
        assert r1.status_code == 200
        assert r2.status_code == 200

    def test_get_sessions_endpoint(self, client):
        tc, headers = client
        r = tc.get('/projects/deek/sessions', headers=headers)
        assert r.status_code == 200
        data = r.json()
    ...
    def test_get_sessions_scoped_endpoint(self, client):
        ...
        tc.post('/projects/deek/subprojects', headers=headers, json={
            'name': 'scope-test.example.com',
            'display_name': 'Scope Test',
        })
        r = tc.get(
            '/projects/deek/sessions',
            headers=headers,
            params={'subproject': 'deek:scope-test.example.com'},
        )
        assert r.status_code == 200
        data = r.json()
        assert 'sessions' in data
...
class TestHybridRetriever:
    def test_cache_key_includes_subproject(self):
        ...
        assert retriever._cache_key(None) == 'deek:global'
        assert retriever._cache_key('deek:demnurse') == 'deek:demnurse'
    ...
    def test_bm25_finds_exact_match(self):
        ...
        class FakeEngine:
            ...
            def get_all_chunks(self, subproject_id=None):
                assert subproject_id == 'deek:demnurse'
                return [
                    {
                        'file': 'stock.py',
...
class TestFilesEndpoint:
    def test_get_files_returns_200(self, client):
        tc, headers = client
        r = tc.get('/projects/deek/files', headers=headers)
        assert r.status_code == 200

    def test_get_files_has_files_key(self, client):
        tc, headers = client
        data = tc.get('/projects/deek/files', headers=headers).json()
        assert 'files' in data
        assert isinstance(data['files'], list)

    def test_get_files_q_filter(self, client):
        tc, headers = client
        data = tc.get('/projects/deek/files?q=agent', headers=headers).json()
...
class TestSymbolsEndpoint:
    def test_get_symbols_returns_200(self, client):
        tc, headers = client
        r = tc.get('/projects/deek/symbols?q=agent', headers=headers)
        assert r.status_code == 200

    def test_get_symbols_has_symbols_key(self, client):
        tc, headers = client
        data = tc.get('/projects/deek/symbols?q=route', headers=headers).json()
        assert 'symbols' in data
        assert isinstance(data['symbols'], list)

    def test_get_symbols_requires_auth(self, client):
        tc, _ = client
        r = tc.get('/projects/deek/symbols?q=foo')
        assert r.status_code == 401
...
class TestModelOverrideRouting:
    ...
    def test_model_override_auto_uses_classifier(self):
        ...
        import os
        from core.models.router import route, ModelChoice
        # With DEEK_FORCE_API=true and no DeepSeek key, should route to API
        os.environ['DEEK_FORCE_API'] = 'true'
...
class TestProcessStreaming:
    """Tests for DeekAgent.process_streaming() async generator."""

    def test_process_streaming_is_async_generator(self):
        """process_streaming() must be an async generator function."""
        import inspect
        from core.agent import DeekAgent
        assert inspect.isasyncgenfunction(DeekAgent.process_streaming)
    ...
    def test_process_streaming_yields_routing_and_complete(self, tmp_path):
        ...
        with patch("core.models.claude_client.ClaudeClient.chat", new_callable=AsyncMock) as mock_chat, \
            ...
            agent = DeekAgent(project_id="test", config={"name": "test", "force_model": "api", "permissions": ["read_file"]})
    ...
    def test_process_streaming_emits_tool_events_for_safe_tool(self, tmp_path):
        ...
        with patch("core.models.claude_client.ClaudeClient.chat", new_callable=AsyncMock) as mock_chat, \
             ...
             patch("core.agent.DeekAgent._execute_tool", new_callable=AsyncMock) as mock_exec:
    ...
    def test_process_streaming_queues_review_tool(self, tmp_path):
        ...
        with patch("core.models.claude_client.ClaudeClient.chat", new_callable=AsyncMock) as mock_chat, \
            ...
            agent = DeekAgent(project_id="test", config={"name": "test", "force_model": "api", "permissions": ["read_file", "edit_file"]})
...
class TestChatStreamEndpoint:
    ...
    def test_chat_stream_endpoint_returns_200(self, client):
        ...
        r = tc.get(
            '/chat/stream',
            headers=headers,
            params={"project": "deek", "session_id": "stream-ep-test", "message": "hello"},
        )
        assert r.status_code == 200
    ...
    def test_chat_stream_content_type_is_sse(self, client):
        ...
        r = tc.get(
            '/chat/stream',
            headers=headers,
            params={"project": "deek", "session_id": "stream-ct-test", "message": "hi"},
        )
    ...
    def test_chat_stream_body_contains_data_prefix(self, client):
        ...
        r = tc.get(
            '/chat/stream',
            headers=headers,
            params={"project": "deek", "session_id": "stream-body-test", "message": "hello"},
        )
    ...
    def test_chat_stream_contains_done_event(self, client):
        ...
        r = tc.get(
            '/chat/stream',
            headers=headers,
            params={"project": "deek", "session_id": "stream-done-test", "message": "hello"},
        )
    ...
    def test_chat_stream_contains_complete_event(self, client):
        ...
        r = tc.get(
            '/chat/stream',
            headers=headers,
            params={"project": "deek", "session_id": "stream-complete-test", "message": "hello"},
        )
    ...
    def test_chat_stream_requires_auth(self, client):
        ...
        r = tc.get(
            '/chat/stream',
            params={"project": "deek", "session_id": "stream-auth-test", "message": "hello"},
        )
        assert r.status_code == 401
...
class TestMemoryAssembler:
    ...
    def test_assembler_respects_ollama_4000_token_budget(self):
        ...
        packet = asyncio.run(assembler.assemble(
            query='test query', project_id='deek', session_id='s1', provider='ollama',
        ))
    ...
    def test_assembler_respects_deepseek_32000_token_budget(self):
        ...
        assembler = self._make_assembler(retriever=retriever)
        packet = asyncio.run(assembler.assemble(
            query='test query', project_id='deek', session_id='s1', provider='deepseek',
        ))
    ...
    def test_assembler_always_includes_core_rules(self):
        ...
        packet = asyncio.run(assembler.assemble(
            query='test', project_id='deek', session_id='s1', provider='sonnet',
        ))
        # core_rules should be populated (deek project has core.md)
    ...
    def test_assembler_trims_retrieved_when_over_budget(self):
        ...
        assembler = self._make_assembler(retriever=retriever)
        packet = asyncio.run(assembler.assemble(
            query='test', project_id='deek', session_id='s1', provider='ollama',
        ))
    ...
    def test_format_for_provider_no_cache_control_deepseek(self):
        assembler = self._make_assembler()
        packet = asyncio.run(assembler.assemble(
            query='test', project_id='deek', session_id='s1', provider='deepseek',
        ))
...
class TestIndexEndpoint:
    ...
    def test_index_endpoint_returns_run_id(self, client):
        ...
        with patch('core.context.indexer.CodeIndexer') as mock_cls:
            ...
            r = tc.post("/projects/deek/index", json={'force': True}, headers=headers)
        assert r.status_code == 200
        data = r.json()
        assert 'run_id' in data
        assert data['run_id'].startswith('idx_')
        assert data['status'] == 'started'
        assert data['project'] == 'deek'
    ...
    def test_index_status_endpoint_returns_progress(self, client):
        ...
        r = tc.get("/projects/deek/index/idx_test123", headers=headers)
        assert r.status_code == 200
        data = r.json()
    ...
    def test_index_status_404_for_unknown_run(self, client):
        tc, headers = client
        r = tc.get("/projects/deek/index/idx_nonexistent", headers=headers)
        assert r.status_code == 404
...
class TestAutoIndexAndScheduled:
    ...
    def test_auto_index_skipped_when_env_var_set(self):
        """DEEK_SKIP_AUTO_INDEX=true should prevent auto-indexing."""
        skip_val = os.environ.get('DEEK_SKIP_AUTO_INDEX') or os.environ.get('CAIRN_SKIP_AUTO_INDEX', '')
        assert skip_val.lower() in {'', '0', 'false', 'no'} or skip_val == '', \
            "DEEK_SKIP_AUTO_INDEX should not be set during tests"
    ...
    def test_scheduled_reindex_disabled_when_zero(self):
        ...
        assert int(os.getenv('DEEK_REINDEX_INTERVAL_HOURS') or os.getenv('CAIRN_REINDEX_INTERVAL_HOURS', '24')) > 0 or True
...
class TestActivityEventsAndSummary:
    ...
    def test_tool_start_includes_tool_name(self, tmp_path):
        ...
        with patch("core.models.claude_client.ClaudeClient.chat", new_callable=AsyncMock) as mc, \
             ...
             patch("core.agent.DeekAgent._execute_tool", new_callable=AsyncMock) as me:
