#!/usr/bin/env python3
"""Quote review cutover — flip DEEK_QUOTE_REVIEW_SHADOW to false.

Scheduled one-shot via cron for 2026-05-13 09:00 UTC. Checks that
the reviewer has run enough to be worth surfacing, then:

  1. Updates /opt/nbne/deek/deploy/.env — DEEK_QUOTE_REVIEW_SHADOW=false
  2. Restarts deek-api so the env takes effect
  3. Writes a cutover record to data/quote_review_cutover.jsonl

Mirrors scripts/impressions_cutover.py and scripts/crosslink_cutover.py.

Safety gates:
    - At least 20 debug rows logged in quote_review_shadow
    - Span of at least 3 days between first and last row
    - Env file exists and is writable
    - deek-api container is running

Usage:
    python scripts/quote_review_cutover.py              # safe mode
    python scripts/quote_review_cutover.py --dry-run    # report only
    python scripts/quote_review_cutover.py --force      # skip gates
"""
...
CUTOVER_LOG = Path(os.getenv(
    'DEEK_QUOTE_REVIEW_CUTOVER_LOG',
    str(REPO_ROOT / 'data' / 'quote_review_cutover.jsonl'),
))
...
log = logging.getLogger('quote_review_cutover')


def read_shadow_stats() -> dict:
    """Count rows + span in cairn_intel.quote_review_shadow."""
    import psycopg2
            ...
            with conn.cursor() as cur:
                cur.execute(
                    """SELECT COUNT(*),
                         ...
                         FROM cairn_intel.quote_review_shadow"""
                )
...
def flip_env_file(env_file: Path) -> bool:
    ...
    if not replaced:
        ...
        out.append(wanted)
    tmp = env_file.with_suffix(env_file.suffix + '.qrcut-tmp')
    try:
...
def main() -> int:
    ...
    log.info('--- quote review shadow stats ---')
    ...
    record['cutover_applied'] = True
    write_cutover_record(record)
    log.info('CUTOVER COMPLETE — DEEK_QUOTE_REVIEW_SHADOW=false')
    return 0
