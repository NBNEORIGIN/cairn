...
DEFAULT_SUITE = ROOT / 'projects' / 'claw' / 'eval_prompt_suite.json'
DEFAULT_CACHE = ROOT / 'data' / 'eval_cache.json'


def main() -> int:
    parser = argparse.ArgumentParser(description='Run the CLAW live answer evaluator.')
    parser.add_argument('--api-url', default='http://localhost:8765')
    parser.add_argument('--api-key', default='claw-dev-key-change-in-production')
    parser.add_argument('--project', default='claw')
