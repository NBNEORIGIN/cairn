"""
Deek cairn@ inbox processor — run by Windows Scheduled Task every 15 minutes.

Checks cairn@nbnesigns.com for new messages, ingests and embeds them.
Handles both forwarded business email and direct notes to Deek.

Usage:
    D:\deek\.venv\Scripts\python.exe D:\deek\scripts\process_deek_inbox.py

Registered by: scripts\install_scheduled_tasks.ps1 (DeekEmailInbox task)
"""
import sys
import json
import logging
...
log_file = log_dir / 'deek_inbox.log'
...
logger = logging.getLogger('deek_inbox_processor')
...
def main():
    ...
    try:
        ...
        result = process_deek_inbox(embed_immediately=True)
