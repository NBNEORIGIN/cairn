...
def _build_reply_back_block(row: dict, include_q5: bool = False) -> list[str]:
    ...
    lines = [
        ...
        '=' * 60,
        '',
        '--- Q0 (email_classification) ---',
        '  What kind of email is this? Reply with ONE of:',
        '    EXISTING       — reply on an existing project (default — also',
        '                     fires if you skip this block and answer Q1)',
        '    SPAM           — junk / marketing / cold sales pitch. Adds the',
        '                     sender DOMAIN to the learned ignore list so',
        '                     future emails from it skip the digest entirely.',
        '    NEW_ENQUIRY    — first contact about a NEW project. Captures the',
        '                     full email body as a training-signal memory so',
        '                     retrieval surfaces it next time. (Note: Cairn',
        '                     does not auto-create a CRM project today — you',
        '                     still raise it manually; this captures the',
        '                     enquiry context for future search.)',
        '    PERSONAL       — personal email that landed in business inbox.',
        '                     Adds sender EMAIL (not domain) to a learned',
        '                     filter so future emails from it skip.',
        '    IGNORE         — not spam, but skip this one. No learning',
        '                     signal recorded.',
        '',
        '  Skip Q0 entirely if this is an existing-project reply — Q1 takes',
        '  over from there. Q0 is for when the auto-classification got it',
        '  meaningfully wrong.',
        '',
        '--- Q1 (match_confirm) ---',
        '  (Only matters if Q0 = EXISTING, the default.)',
